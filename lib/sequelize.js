...
class Sequelize {
   ...
   * @param {string|boolean} [options.clientMinMessages='warning'] The Postgres `client_min_messages` session parameter. Set to `false` to not override the database's default
   * @param {boolean}  [options.standardConformingStrings=true] The Postgres `standard_conforming_strings` session parameter. Set to `false` to not set the option. WARNING: Setting this to false may expose vulnerabilities and is not recommended!
   ...
   * @param {string}   [options.deferrable] Sets the constraints to be deferred or immediately checked. See `Sequelize.Deferrable`. Postgres Only
   * @param {Function} [options.logging=false] A function that gets executed while running the query to log the sql
